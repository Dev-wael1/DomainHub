...
export default function HeroForm_5({
  data = { enable: false },
  homePage,
  aiGenerator,
  whois,
  ...
  return (
    <>
      {data?.enable && (
        <>
          {/* <div className="whoIsHostingB bg-gradien anim-bg3 "></div> */}
          {/* Hero */}
          <div className="overflow-hidden min-h-screen anim-bg ">
            <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 lg:py-20 2xl:py-52">
              <div className="relative mx-auto max-w-4xl grid space-y-5 sm:space-y-10">
                <div className="text-center">
                  <p className="text-xs font-semibold text-gray-100 tracking-wide uppercase mb-3 dark:text-gray-100">
                    {data?.topTitle || "Small business solutions"}
                  </p>
                  <h1 className="text-3xl text-gray-100 font-bold sm:text-5xl lg:text-7xl lg:leading-tight dark:text-gray-100">
                    {data?.mainTitle?.split(".")[0] ||
                      "Turn online shoppers into"}{" "}
                  </h1>
                  <h1 className="text-3xl sm:text-5xl lg:text-6xl font-bold lg:leading-light text-transparent bg-clip-text bg-gradient-to-r from-[#4F46E5] to-[#E114E5]">
                    {data?.mainTitle?.split(".")[1] || "WordPress Theme"}
                  </h1>
                </div>
                {/* End Title */}
                {/* Avatar Group */}
                ...
                <div className="sm:flex sm:justify-center sm:items-center text-center sm:text-left">
                  <div className="flex-shrink-0 pb-5 sm:flex sm:pb-0 sm:pr-5">
                    <div className="flex justify-center -space-x-3">
                      <span className="inline-flex items-center justify-center h-8 w-8 rounded-full ring- ring-white bg-violet-600 dark:bg-gray-900 dark:ring-gray-800">
                  ...
                  <div className="pt-5 sm:pt-0 sm:pl-5">
                    <div className="text-lg font-semibold text-gray-100 ">
                      {data?.rightTitle || "Website Checked"}
                    </div>
                    {/* <div className="text-sm text-gray-800">
                      {wpHostingPageData?.hero?.rightDescription}
                    </div> */}
                  </div>
                </div>

                <form onSubmit={handleSubmit}>
                  <div className="mx-auto max-w-4xl sm:flex sm:space-x-3 p-3 bg-gray-300 rounded-3xl shadow-lg  dark:bg-gray-900 dark:border-gray-700 dark:shadow-gray-900/[.2] flex flex-col sm:flex-row align-center">
                    <input
                      required
                      value={value}
                      onChange={(e) => setValue(e.target.value)}
                      type="text"
                      className="block w-full p-3 text-gray-600 dark:text-gray-200 bg-transparent border border-transparent appearance-none rounded-3xl focus:border-slate-500 focus:outline-none focus:ring-slate-500 dark:placeholder:text-slate-300 sm:text-lg"
                    ...
                    <div className="pt-2 sm:pt-0 grid   sm:block sm:flex-[0_0_auto]">
                      <button
                        type="submit"
                        className="py-3 px-4 inline-flex justify-center items-center gap-2  border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm sm:py-5 sm:px-8 dark:focus:ring-offset-gray-800 rounded-3xl"
                        // href="#"
                      >
