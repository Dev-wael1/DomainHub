...
export default function AdminFeatures_7({ storagePath, dbCollection }) {
  ...
  const handleSave = (e) => {
    setLoading(true);
    axios
      .post("/api/features/features-7", formData)
      .then((res) => {
        setLoading(false);
  ...
  useEffect(() => {
    axios
      .get("/api/features/features-7", { params: { dbCollection } })
      .then((res) => {
        if (res.data?.features_7) {
          setEnable(res.data?.features_7?.enable);
          setTabTitle(res.data?.features_7?.tabTitle);

          setTab1Title(res.data?.features_7?.tab1Title);
          setTab1Desc(res.data?.features_7?.tab1Desc);

          setTab2Title(res.data?.features_7?.tab2Title);
          setTab2Desc(res.data?.features_7?.tab2Desc);

          setTab3Title(res.data?.features_7?.tab3Title);
          setTab3Desc(res.data?.features_7?.tab3Desc);
        }
      });
  }, []);
