...
export default function HeroAdmin_14({ storagePath, dbCollection }) {
  ...
  const handleSave = (e) => {
    ...
    setLoading(true);
    axios
      .post("/api/hero/hero-14", formData)
      .then((res) => {
        setLoading(false);
  ...
  useEffect(() => {
    axios.get("/api/hero/hero-14", { params: { dbCollection } }).then((res) => {
      if (res.data?.hero_14) {
        setEnable(res.data?.hero_14?.enable);
        setTitle(res.data?.hero_14?.title);
        setDesc(res.data?.hero_14?.desc);
        setLeftButtonText(res.data?.hero_14?.leftButtonText);
        setLeftButtonLink(res.data?.hero_14?.leftButtonLink);
        setRightButtonText(res.data?.hero_14?.rightButtonText);
        setRightButtonLink(res.data?.hero_14?.rightButtonLink);
      }
    });
  }, []);
