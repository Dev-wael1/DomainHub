...
import Hero_16 from "../../Front/Hero/Hero_16";

export default function HeroAdmin_16({ storagePath, dbCollection }) {
  ...
  const handleSave = (e) => {
    ...
    setLoading(true);
    axios
      .post("/api/hero/hero-16", formData)
      .then((res) => {
        setLoading(false);
  ...
  useEffect(() => {
    axios.get("/api/hero/hero-16", { params: { dbCollection } }).then((res) => {
      if (res.data?.hero_16) {
        setEnable(res.data?.hero_16?.enable);
        setTitle(res.data?.hero_16?.title);
        setDesc(res.data?.hero_16?.desc);
        setLeftButtonText(res.data?.hero_16?.leftButtonText);
        setLeftButtonLink(res.data?.hero_16?.leftButtonLink);
        setRightButtonText(res.data?.hero_16?.rightButtonText);
        setRightButtonLink(res.data?.hero_16?.rightButtonLink);
      }
    });
  }, []);
