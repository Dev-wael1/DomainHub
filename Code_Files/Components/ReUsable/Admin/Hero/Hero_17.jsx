...
import Hero_17 from "../../Front/Hero/Hero_17";

export default function HeroAdmin_17({ storagePath, dbCollection }) {
  ...
  const handleSave = (e) => {
    ...
    setLoading(true);
    axios
      .post("/api/hero/hero-17", formData)
      .then((res) => {
        setLoading(false);
  ...
  useEffect(() => {
    axios.get("/api/hero/hero-17", { params: { dbCollection } }).then((res) => {
      if (res.data?.hero_17) {
        setEnable(res.data?.hero_17?.enable);
        setTitle(res.data?.hero_17?.title);
        setDesc(res.data?.hero_17?.desc);
        setLeftButtonText(res.data?.hero_17?.leftButtonText);
        setLeftButtonLink(res.data?.hero_17?.leftButtonLink);
        setRightButtonText(res.data?.hero_17?.rightButtonText);
        setRightButtonLink(res.data?.hero_17?.rightButtonLink);
      }
    });
  }, []);
