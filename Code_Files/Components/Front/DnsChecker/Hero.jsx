...
import { setLoading, setDnsInfo } from "../../../Redux/reducer";
export default function Hero() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const serverLocation = document.getElementById("serverLocation")?.offsetTop;
    window.scrollTo({ top: serverLocation, behavior: "smooth" });
    dispatch(setLoading(true));
    axios
      .post("/api/dns-checker/get-dns", { inputValue })
      .then((res) => {
        // dispatch(setWhoisHostingInputValue(inputValue));
  ...
  return (
    <>
      <div className="whoIsHostingBg absolute blur-3xl"></div>
      {/* Hero */}
        ...
        <div className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="relative mx-auto max-w-4xl grid space-y-5 sm:space-y-10">
            ...
            <div className="text-center">
              <p className="text-xs font-semibold text-gray-700 tracking-wide uppercase mb-3 dark:text-gray-900">
                {dnsCheckerPageData?.hero?.topTitle ||
                  "Small business solutions"}
              </p>
              <h1 className="text-3xl text-gray-800 font-bold sm:text-5xl lg:text-6xl lg:leading-tight dark:text-gray-900">
                {dnsCheckerPageData?.hero?.mainTitle?.split(".")[0] ||
                  "Turn online shoppers into"}{" "}
              </h1>
              <h1 className="text-3xl text-gray-800 font-bold sm:text-5xl lg:text-6xl lg:leading-tight dark:text-gray-900 text-transparent bg-clip-text bg-gradient-to-r from-[#4F46E5] to-[#E114E5]">
                {dnsCheckerPageData?.hero?.mainTitle?.split(".")[1] ||
                  "Turn online shoppers into"}
              </h1>
            </div>
            {/* End Title */}
            ...
            <div className="sm:flex sm:justify-center sm:items-center text-center sm:text-left">
                ...
                <div className="flex justify-center -space-x-3">
                  <span className="inline-flex items-center justify-center h-8 w-8 rounded-full ring-2 ring-white bg-gray-800 dark:bg-gray-900 dark:ring-gray-800">
                    <span className="text-xs font-medium leading-none text-white uppercase">
                      {dnsCheckerPageData?.hero?.leftNumber || "7k+"}
                    </span>
                  </span>
                </div>
                {/* End Avatar Group */}
              ...
              <div className="pt-5 sm:pt-0 sm:pl-5">
                <div className="text-lg font-semibold text-gray-800 dark:text-gray-900">
                  {dnsCheckerPageData?.hero?.rightTitle || "Hosting"}
                </div>
                <div className="text-sm text-gray-700">
                  {dnsCheckerPageData?.hero?.rightDescription ||
                    "Server Location Checked"}
                </div>
              </div>
            </div>
            {/* End Avatar Group */}
            ...
            <form onSubmit={handleSubmit}>
              <div className="mx-auto max-w-2xl sm:flex sm:space-x-3 p-3 bg-white border rounded-xl shadow-lg  dark:bg-slate-900 dark:border-gray-700 dark:shadow-gray-900/[.2] flex flex-col sm:flex-row align-center">
                <input
                  required
                  value={inputValue}
                  onChange={(e) => setInputValue(e.target.value)}
                  type="text"
                  className="py-3 px-4 block w-full border-transparent rounded-xl text-lg focus:border-yellow-500 focus:ring-yellow-500 sm:p-4 dark:bg-slate-900 dark:border-red-500 dark:text-gray-400"
                ...
                <div className="pt-2 sm:pt-0 grid   sm:block sm:flex-[0_0_auto]">
                  <button
                    type="submit"
                    className="py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm sm:py-5 sm:px-8 dark:focus:ring-offset-gray-800 rounded-xl"
                    // href="#"
                  >
                    Get Location
                  </button>
